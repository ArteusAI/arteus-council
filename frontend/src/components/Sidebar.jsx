...
export default function Sidebar({
  ...
  const busyHintTimerRef = useRef(null);

  useEffect(() => {
    if (!leadsAnalysisBusy && showBusyHint) {
      setShowBusyHint(false);
    }
  }, [leadsAnalysisBusy, showBusyHint]);
  ...
  const handleNewConversationClick = () => {
    if (leadsAnalysisBusy) {
      setShowBusyHint(true);
      if (busyHintTimerRef.current) clearTimeout(busyHintTimerRef.current);
      busyHintTimerRef.current = setTimeout(() => setShowBusyHint(false), 5000);
      return;
    }
    onNewConversation();
  };
    ...
    <div className={`sidebar ${isOpen ? 'open' : ''}`}>
      ...
      <div className="sidebar-header">
        ...
        <div className="new-conversation-wrapper">
          <button
            className={`new-conversation-btn${leadsAnalysisBusy ? ' new-conversation-btn-busy' : ''}`}
            onClick={handleNewConversationClick}
            aria-disabled={leadsAnalysisBusy}
            title={leadsAnalysisBusy ? t('leadsAnalysisBusy') : undefined}
          >
            + {t('newConversation')}
          </button>
          {showBusyHint && leadsAnalysisBusy && (
            <div className="new-conversation-busy-hint" role="status">
              {t('leadsAnalysisBusyHint')
                || 'An analysis is currently running. Close the current chat to stop it and start a new one.'}
            </div>
          )}
        </div>
      </div>
            ...
            {conversations.map((conv) => {
              ...
              return (
                <div
                  ...
                  } ${hasActiveJob && !leadsMode ? 'has-progress' : ''}`}
      ...
      <div className="sidebar-footer">
        ...
        {leadsMode ? (
          <div className="footer-controls leads-personalization-row">
            <button
              className="personalization-btn personalization-btn-inline"
